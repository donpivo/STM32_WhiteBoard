...
void setup() {
  ...
  uart2.println(F("Adafruit SPI Flash FatFs Full Usage Example"));

  // Initialize flash library and check its chip ID.
  if (!flash.begin()) {
    uart2.println(F("Error, failed to initialize flash chip!"));
    while (1) {
      yield();
    }
  }
  uart2.print(F("Flash chip JEDEC ID: 0x"));
  uart2.println(flash.getJEDECID(), HEX);

  // First call begin to mount the filesystem.  Check that it returns true
  // to make sure the filesystem was mounted.
  if (!fatfs.begin(&flash)) {
    uart2.println(F("Error, failed to mount newly formatted filesystem!"));
    uart2.println(
  ...
  uart2.println(F("Mounted filesystem!"));
  ...
  if (!fatfs.exists(D_TEST)) {
    uart2.println(F("Test directory not found, creating..."));
    ...
    if (!fatfs.exists(D_TEST)) {
      uart2.println(F("Error, failed to create directory!"));
      while (1) {
        yield();
      }
    } else {
      uart2.println(F("Created directory!"));
    }
  }
  ...
  uart2.println(F("Creating deep folder structure..."));
  if (!fatfs.exists(D_TEST_FOO_BAR)) {
    uart2.println(F("Creating " D_TEST_FOO_BAR));
  ...
  if (!fatfs.exists(D_TEST_FOO_BAZ)) {
    uart2.println(F("Creating " D_TEST_FOO_BAZ));
  ...
  if (!writeFile) {
    uart2.println(F("Error, failed to open " F_TEST_TEST_TXT " for writing!"));
    while (1) {
      yield();
    }
  }
  uart2.println(F("Opened file " F_TEST_TEST_TXT " for writing/appending..."));
  ...
  writeFile.close();
  uart2.println(F("Wrote to file " F_TEST_TEST_TXT "!"));

  // Now open the same file but for reading.
  File32 readFile = fatfs.open(F_TEST_TEST_TXT, FILE_READ);
  if (!readFile) {
    uart2.println(F("Error, failed to open " F_TEST_TEST_TXT " for reading!"));
    while (1) {
      yield();
    }
  }
  ...
  uart2.print(F("First line of test.txt: "));
  uart2.println(line);

  // You can get the current position, remaining data, and total size of the
  // file:
  uart2.print(F("Total size of test.txt (bytes): "));
  uart2.println(readFile.size(), DEC);
  uart2.print(F("Current position in test.txt: "));
  uart2.println(readFile.position(), DEC);
  uart2.print(F("Available data to read in test.txt: "));
  uart2.println(readFile.available(), DEC);

  // And a few other interesting attributes of a file:
  char readName[64];
  readFile.getName(readName, sizeof(readName));
  uart2.print(F("File name: "));
  uart2.println(readName);
  uart2.print(F("Is file a directory? "));
  uart2.println(readFile.isDirectory() ? F("Yes") : F("No"));

  // You can seek around inside the file relative to the start of the file.
  // For example to skip back to the start (position 0):
  if (!readFile.seek(0)) {
    uart2.println(F("Error, failed to seek back to start of file!"));
    while (1) {
      yield();
    }
  }
  ...
  uart2.println(F("Entire contents of test.txt:"));
  ...
  if (!testDir) {
    uart2.println(F("Error, failed to open test directory!"));
    while (1) {
      yield();
    }
  }
  if (!testDir.isDirectory()) {
    uart2.println(F("Error, expected test to be a directory!"));
    while (1) {
      yield();
    }
  }
  uart2.println(F("Listing children of directory " D_TEST ":"));
  ...
  while (child) {
    ...
    uart2.print(F("- "));
    uart2.print(filename);
    if (child.isDirectory()) {
      uart2.print(F(" (directory)"));
    }
    uart2.println();
  ...
  test2File.close();
  uart2.println(F("Deleting " F_TEST_FOO_TEST2_TXT "..."));
  if (!fatfs.remove(F_TEST_FOO_TEST2_TXT)) {
    uart2.println(F("Error, couldn't delete " F_TEST_FOO_TEST2_TXT " file!"));
    while (1) {
      yield();
    }
  }
  uart2.println(F("Deleted file!"));
  ...
  // unix filesystems!
  uart2.println(F("Deleting /test directory and everything inside it..."));
  if (!testDir.rmRfStar()) {
    uart2.println(F("Error, couldn't delete test directory!"));
    while (1) {
      yield();
    }
  }
  ...
  if (fatfs.exists(D_TEST)) {
    uart2.println(F("Error, test directory was not deleted!"));
    while (1) {
      yield();
    }
  }
  uart2.println(F("Test directory was deleted!"));

  uart2.println(F("Finished!"));
}
